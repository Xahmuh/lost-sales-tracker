...
const StrategicKPI: React.FC<{
  ...
  return (
    <div className={`p-8 rounded-[2rem] border shadow-sm transition-all duration-500 relative flex flex-col justify-between min-h-[180px] ${isPrimary
      ? 'bg-red-900 border-none text-white overflow-hidden'
      : critical
        ? 'bg-white border-red-200 text-slate-900 group shadow-lg shadow-red-500/5'
        : 'bg-white border-slate-100 text-slate-900 group hover:border-red-200'
      }`}>
      {isPrimary && (
      ...
      {critical && !isPrimary && (
        <div className="absolute top-4 left-4 flex items-center gap-1.5 px-3 py-1 bg-red-50 border border-red-100 rounded-full animate-pulse shadow-sm">
      ...
      <div className={`flex items-start justify-between relative z-10 ${critical ? 'mt-4' : ''}`}>
        <div>
          <h3 className={`text-[11px] font-black uppercase tracking-[0.2em] ${isPrimary ? 'text-white/60' : 'text-slate-400'}`}>
            {label}
          </h3>
          {description && (
            <p className="mt-1 text-[11px] font-bold leading-tight" style={{ color: '#1f161b' }}>
              "{description}"
            </p>
          )}
        </div>
        <div className={`w-10 h-10 rounded-full flex items-center justify-center border shrink-0 ${isPrimary
          ? 'bg-white/10 border-white/20 text-white'
          : critical
            ? 'bg-red-50 border-red-100 text-red-600'
            : 'bg-slate-50 border-slate-100 text-slate-400 group-hover:text-red-500 group-hover:border-red-100'
          }`}>
          {icon}
        </div>
      </div>
      ...
      <div className="mt-4 relative z-10 flex flex-col">
        <div className="flex items-baseline gap-2">
          {(isCurrency || unit) && (
            <span className={`text-sm font-black tracking-tighter ${isPrimary ? 'text-white/40' : 'text-red-500/40'}`}>
              {isCurrency ? 'BHD' : unit}
            </span>
          )}
          <span className={`text-5xl font-black tracking-tighter tabular-nums ${critical && !isPrimary ? 'text-red-700' : ''}`}>
            {value}
          </span>
        </div>
...
export const DashboardPage: React.FC<DashboardPageProps> = ({ user }) => {
  ...
  }, [selectedBranch, dateType, startDate, endDate, isAdmin, user.id, user.role]);
  ...
  const todaySales = useMemo(() => {
    const threshold = new Date(); threshold.setHours(0, 0, 0, 0);
    return allSales.filter(s => new Date(s.timestamp) >= threshold);
  }, [allSales]);
  ...
  const performanceTrend = useMemo(() => {
    const trendMap: Record<string, { value: number, count: number, sessions: Set<string> }> = {};
    sales.forEach(s => {
      ...
      if (!trendMap[dateStr]) trendMap[dateStr] = { value: 0, count: 0, sessions: new Set() };
      trendMap[dateStr].value += Number(s.totalValue);
      trendMap[dateStr].sessions.add(sessionId);
    });
    ...
    while (current <= trendEndDate) {
      ...
      const data = trendMap[dateStr] || { value: 0, count: 0, sessions: new Set() };
      timeline.push({
        name: dateStr,
        value: data.value,
  ...
  const exportLostSales = async () => {
    ...
    try {
      ...
      if (!viewData || viewData.length === 0) {
        alert("No data passed the filter to export.");
        return;
      }
      ...
      alert("Extraction failed. Connection or Permission issue.");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const exportShortage = async () => {
    ...
    try {
      ...
      if (error) throw error;
      if (!viewData || viewData.length === 0) {
        alert("No shortages found for this period.");
        return;
      }
    ...
    <div className="min-h-screen bg-white font-sans selection:bg-red-100">
      <div className="max-w-[1600px] mx-auto px-6 md:px-12 py-10">
            ...
            {isAdmin && (
              <div className="relative">
                ...
                {isBranchDropdownOpen && (
                  <div className="absolute top-full right-0 mt-3 w-72 bg-white rounded-[2rem] shadow-2xl border border-slate-100 p-3 z-[100] animate-in zoom-in-95 duration-300">
                    <div className="max-h-80 overflow-y-auto custom-scrollbar space-y-1">
                      <button onClick={() => { setSelectedBranch('all'); setIsBranchDropdownOpen(false); }} className={`w-full text-left p-4 rounded-xl text-[10px] font-black uppercase tracking-widest ${selectedBranch === 'all' ? 'bg-red-900 text-white' : 'hover:bg-slate-50'}`}>All Global Branches</button>
                      {branches.map(b => (
                        <button key={b.id} onClick={() => { setSelectedBranch(b.id); setIsBranchDropdownOpen(false); }} className={`w-full text-left p-4 rounded-xl text-[10px] font-black uppercase tracking-widest ${selectedBranch === b.id ? 'bg-red-900 text-white' : 'hover:bg-slate-50'}`}>{b.name}</button>
                      ))}
                    </div>
                  </div>
                )}
        ...
        <div className="flex justify-center mb-12">
          <div className="bg-white p-1.5 rounded-full border border-slate-100 shadow-sm inline-flex">
            <button
          ...
          viewMode === 'standard' ? (
            <>
              ...
              <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-24">
                ...
                <StrategicKPI label="LOST CUSTOMERS NO" value={aggregateMetrics.incidentCount} icon={<AlertCircle size={20} />} critical={true} />
                ...
                <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
                  ...
                  <div className="bg-slate-900 p-6 rounded-[2rem] border border-slate-800 shadow-xl flex flex-col items-center text-center group hover:-translate-y-1 transition-all duration-500">
                    ...
                    <div className="mt-4 w-full h-1 bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full bg-emerald-500 transition-all duration-1000" style={{ width: `${aggregateMetrics.altPercentage}%` }}></div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-900 p-6 rounded-[2rem] border border-slate-800 shadow-xl flex flex-col items-center text-center group hover:-translate-y-1 transition-all duration-500">
                    ...
                    <div className="mt-4 w-full h-1 bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full bg-emerald-500 transition-all duration-1000" style={{ width: `${aggregateMetrics.transferPercentage}%` }}></div>
                    </div>
                  </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    ...
                    <tbody className="text-sm font-bold text-slate-700">
                      {todaySales.slice(0, 10).map((s, idx) => (
                        <tr key={s.id || idx} className="border-b border-slate-50 hover:bg-slate-50/50 transition-all">
                          <td className="py-4 pl-4">
                            <p className="text-slate-900 font-black uppercase text-xs truncate max-w-[200px]">{s.productName}</p>
                          ...
                          <td className="py-4 text-right text-slate-900 pr-4 tabular-nums">
                  ...
                  {todaySales.length === 0 && (
                    <div className="text-center py-12">
                      <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">No lost sales today</p>
                    </div>
                  )}
                </div>
              </section>
...

        {
          viewMode === 'expanded' ? (
            <>
                ...
                <div className="px-8 py-6 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-50/30">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all duration-300 ${shortageStatusFilter === 'Low'
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all duration-300 ${shortageStatusFilter === 'Critical'
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all duration-300 ${shortageStatusFilter === 'Out of Stock'
                  ...
                  {shortages.length === 0 ? (
                    ...
                    <>
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                            ...
                            <th className="pb-4 pr-4 text-right">Correlation</th>
                          </tr>
                        </thead>
                        <tbody className="text-sm font-bold text-slate-700">
                          {[...shortages]
                            .filter(s => !shortageStatusFilter || s.status === shortageStatusFilter)
                              ...
                              return (
                                <React.Fragment key={item.id || idx}>
                                  <tr
                                    ...
                                    className={`group hover:bg-slate-50 transition-colors cursor-pointer border-b border-slate-50 relative ${hasLostSale ? 'bg-red-50/10' : ''} ${isExpanded ? 'bg-slate-50' : ''}`}
                                  >
                                    <td className="py-4 pl-4">
                                      <div className="font-black text-slate-900">{item.productName}</div>
                                      <div className="text-[9px] text-slate-400 uppercase tracking-wider mt-0.5">{branches.find(b => b.id === item.branchId)?.name || 'Unknown Branch'}</div>
                                      {isExpanded && <div className="absolute left-0 top-0 bottom-0 w-1 bg-brand rounded-r-full"></div>}
                                    </td>
                                    <td className="py-4">
                                      <span className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider inline-flex items-center gap-2
                             ${item.status === 'Low' ? 'bg-yellow-100 text-yellow-800' :
                                          item.status === 'Critical' ? 'bg-red-100 text-red-800' :
                                            'bg-slate-900 text-white'}`}>
                                        <span className={`w-1.5 h-1.5 rounded-full ${item.status === 'Out of Stock' ? 'bg-white animate-pulse' : 'bg-current'}`}></span>
                                        {item.status}
                                      </span>
                                    </td>
                                    <td className="py-4">
                                      <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs">{item.pharmacistName}</span>
                                      </div>
                                    </td>
                                    ...
                                    <td className="py-4 pr-4">
                                      <div className="flex items-center justify-end gap-3">
                                        {hasLostSale ? (
                                          ...
                                          <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest opacity-50">Safe</span>
                                        )}
                                        <button
                                          onClick={(e) => {
                                            e.stopPropagation();
                                  ...
                                  {isExpanded && (
                                    <tr className="bg-slate-50/50 animate-in fade-in duration-300">
                                      <td colSpan={5} className="p-0 border-b border-slate-50">
                      ...
                      <div className="mt-6 pt-6 border-t border-slate-50 flex items-center justify-between px-2">
                        <button onClick={() => setBranchPage(p => Math.max(1, p - 1))} className="p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-900 hover:text-white transition-all shadow-sm">
                          <ChevronLeft size={18} />
                        </button>
                        <div className="flex flex-col items-center">
                          <span className="text-[9px] font-black text-slate-900 uppercase tabular-nums">
                            Page {branchPage} OF {Math.max(1, Math.ceil(shortages.filter(s => !shortageStatusFilter || s.status === shortageStatusFilter).length / 10))}
                          </span>
                        </div>
                        <button onClick={() => setBranchPage(p => Math.min(Math.ceil(shortages.filter(s => !shortageStatusFilter || s.status === shortageStatusFilter).length / 10), p + 1))} className="p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-900 hover:text-white transition-all shadow-sm">
                  ...
                  <div className="flex items-center justify-between mb-10">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <div className="px-5 py-2.5 bg-slate-900 text-white rounded-xl text-[10px] font-black tabular-nums shadow-lg">
                        BATCH {hotShortagePage} / {Math.ceil(shortageMetrics.topProducts.length / 5) || 1}
                      </div>
                      <button
                        onClick={() => setHotShortagePage(p => Math.min(Math.ceil(shortageMetrics.topProducts.length / 5), p + 1))}
                    ...
                    {shortageMetrics.topProducts.slice((hotShortagePage - 1) * 5, hotShortagePage * 5).map((p, i) => (
                      <div key={i} className="flex items-center justify-between p-6 bg-slate-50/50 rounded-[2rem] border border-slate-100 hover:border-brand hover:bg-white transition-all group cursor-default">
                        <div className="flex items-center space-x-6">
                          <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center font-black text-slate-600 border border-slate-100 shadow-sm text-sm group-hover:scale-110 transition-transform duration-500">
              ...
              <section className="w-full bg-white rounded-[2.8rem] border border-slate-100 shadow-sm overflow-hidden group transition-all duration-500 mb-20 p-8 md:p-12 relative">
...

        {
          viewMode === 'standard' ? (
            <>
                    ...
                    {temporalMatrix.map((h, i) => {
                      ...
                      return (
                        <div key={i} className="group/cell relative aspect-square">
                          <div className={`w-full h-full rounded-[1.5rem] flex flex-col items-center justify-center border transition-all duration-500 hover:scale-105 active:scale-95 cursor-pointer shadow-xl ${ratio > 0.8 ? 'border-brand/40 shadow-brand/10' : 'border-white/[0.03] hover:border-white/10'}`}
                            style={{
              ...
              <section className="w-full bg-white rounded-[2.8rem] border border-slate-100 shadow-sm overflow-hidden group transition-all duration-500 mb-20">
                <div className="px-8 py-6 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-50/30">
                  <div className="flex items-center">
                    <div className="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center text-white mr-4 group-hover:bg-brand transition-colors">
