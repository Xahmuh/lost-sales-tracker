-- ==========================================================
-- ANTIGRAVITY PATCH: DROP FUNCTIONS AND RECREATE (FIX 42P13)
-- ==========================================================

-- 1. DROP EXISTING FUNCTIONS (Required to change return signature)
...
-- 2. RECREATE generate_spin_session (With strict output names)
...
-- 3. RECREATE execute_spin_transaction (Removed prize_value from return as requested)
...
RETURNS TABLE (
    ...
    prize_name TEXT,
    prize_type TEXT
    -- prize_value REMOVED per user request
) LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
    v_branch_id UUID;
    v_customer_id UUID;
...
BEGIN
    ...
    -- 7. Return Results (Without Value)
    RETURN QUERY SELECT 
        v_new_spin_id::UUID, 
        v_local_voucher_code::TEXT, 
        v_prize_id::UUID, 
