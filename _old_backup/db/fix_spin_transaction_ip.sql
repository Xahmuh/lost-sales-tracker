-- ==========================================================
-- ANTIGRAVITY PATCH: FIX RPC SIGNATURE (ADD IP ADDRESS)
-- ==========================================================

-- 1. Ensure spins table has ip_address column
ALTER TABLE public.spins ADD COLUMN IF NOT EXISTS ip_address TEXT;

-- 2. Update the function to accept p_ip_address and persist it
...
BEGIN
    ...
    WHILE EXISTS (SELECT 1 FROM public.spins WHERE voucher_code = v_voucher_code) LOOP
        v_random_suffix := upper(substring(md5(random()::text), 1, 8));
        v_voucher_code := 'VOUCH-' || v_random_suffix;
    END LOOP;
    ...
    VALUES (v_customer_id, v_branch_id, v_prize_id, v_voucher_code, p_ip_address)
