...
interface ManualProductModalProps {
  ...
  initialName: string;
  onClose: () => void;
  onSave: (data: { product_name: string; selling_price: number; agent_name: string; category: string }) => void;
}
      ...
      <div className="bg-white w-full max-w-2xl rounded-[3.5rem] p-12 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.3)] border border-white relative overflow-hidden animate-in zoom-in-95 duration-500">
        ...
        <div className="space-y-8 mb-12 relative z-10">
          <div className="space-y-2">
            ...
            <input
              className="w-full bg-slate-50 border border-slate-100 rounded-[1.5rem] p-6 text-lg font-black text-slate-800 outline-none focus:ring-8 focus:ring-red-500/5 focus:border-[#8B0000] transition-all"
              value={name}
          ...
          <div className="grid grid-cols-2 gap-8">
            <div className="space-y-2">
              ...
              <input
                className="w-full bg-slate-50 border border-slate-100 rounded-[1.5rem] p-6 font-black text-slate-800 outline-none focus:ring-8 focus:ring-red-500/5 focus:border-[#8B0000] transition-all"
                value={agent}
        ...
        <div className="flex relative z-10">
          <button
            onClick={() => onSave({ product_name: name, selling_price: price, agent_name: agent, category })}
            className="flex-1 bg-[#8B0000] hover:bg-[#a00000] text-white font-black py-7 rounded-[2.25rem] shadow-2xl shadow-red-200 transition-all flex items-center justify-center space-x-4 active:scale-95 group"
          >
