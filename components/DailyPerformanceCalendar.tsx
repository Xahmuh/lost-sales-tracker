...
export const DailyPerformanceCalendar: React.FC<DailyPerformanceCalendarProps> = ({ sales }) => {
  ...
  return (
    <div className="bg-white rounded-[3rem] p-12 border border-slate-100 shadow-sm relative overflow-hidden h-full flex flex-col">
      ...
      <div className="flex flex-col gap-8 mb-12 relative z-10">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center bg-slate-50 p-2 rounded-2xl border border-slate-100">
            <button onClick={() => changeMonth(-1)} className="p-3 hover:bg-white hover:text-brand hover:shadow-xl rounded-xl transition-all text-slate-400">
        ...
        <div className="grid grid-cols-7 gap-px bg-slate-100 border border-slate-100 rounded-[2rem] overflow-hidden flex-1 shadow-inner">
          {daysInMonth.map((dateObj, idx) => {
            ...
            return (
              <div 
                key={idx} 
                className={`p-5 transition-all duration-300 group flex flex-col justify-between hover:z-10 hover:shadow-2xl relative ${!dateObj.isCurrentMonth ? 'opacity-10 pointer-events-none' : intensity}`}
              >
                ...
                <div className="mt-auto pt-4">
                  {total > 0 ? (
                    <div className="animate-in fade-in slide-in-from-bottom-2">
                      <p className="text-xs font-black text-slate-900 tracking-tight leading-none mb-1.5">
                        {total.toFixed(3)}
                      </p>
                      <p className="text-[7px] font-black text-brand uppercase tracking-[0.2em] leading-none">BHD Value</p>
                    </div>
                  ) : (
      ...
      <div className="mt-12 pt-10 border-t border-slate-50 flex items-center justify-center gap-10 opacity-30">
        <div className="flex items-center space-x-3">
          <div className="w-3.5 h-3.5 bg-brand rounded-md"></div>
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">IDENTIFIED LOSS</span>
        </div>
        <div className="flex items-center space-x-3">
          <div className="w-3.5 h-3.5 bg-slate-50 rounded-md border border-slate-200"></div>
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Base State</span>
        </div>
      </div>
    </div>
  );
